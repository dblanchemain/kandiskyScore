ardour {
  ["type"]    = "EditorAction",
  name        = "HOA AmbiX – Sparta ambiBIN",
  author      = "D.Blanchemain / Claude",
  description = "Importe partition_ambiX.wav (HOA) et ajoute Sparta ambiBIN (binaural casque). Config lue depuis ~/.config/kandiskyscore/kandiskyscore_hoa.txt",
}
...
function factory () return function ()
  ...
  if not file then
    ...
    print("Lancez d'abord l'export HOA depuis KandiskyScore.")
    return
  end
  ...
  print("=== Import HOA AmbiX – Sparta ambiBIN ===")
  ...
  -- ── Ajout du plugin Sparta ambiBIN (VST2/lxvst) ──────────────
  local proc = ARDOUR.LuaAPI.new_plugin(Session, "sparta_ambiBIN", ARDOUR.PluginType.LXVST, "")

  if proc:isnil() then
    proc = ARDOUR.LuaAPI.new_plugin(Session, "ambiBIN", ARDOUR.PluginType.LXVST, "")
  end

  if proc:isnil() then
    print("⚠ Sparta ambiBIN introuvable.")
    ...
    if ok then
      print("sparta_ambiBIN ajouté.")
      -- masterOrder = auto (détecte depuis le nombre de canaux reçus)
      -- useRoomIR = désactivé (HRTF intégré par défaut)
      -- Pour utiliser un fichier SOFA : bouton « Load SOFA » dans l'interface du plugin
      print("HRTF : intégré par défaut. Pour SOFA personnalisé : bouton Load SOFA dans le plugin.")
    else
      print("⚠ Impossible d'ajouter ambiBIN à la piste.")
    end
  end
  ...
  print("✅ Import HOA Sparta ambiBIN terminé.")
