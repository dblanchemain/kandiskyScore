...
def cmd_ui(uri, initial_str=None):
    ...
    if ui_type_b == 'gtk2':
        # GTK2 et GTK3 incompatibles dans le même processus — déléguer à lv2_ui_gtk2.py
        _dbg('UI GTK2 détectée — délégation à lv2_ui_gtk2.py')
        _L.lilv_world_free(world)
        import subprocess
        script = os.path.join(_script_dir, 'lv2_ui_gtk2.py')
        args   = [sys.executable, script, uri]
        if initial_str:
