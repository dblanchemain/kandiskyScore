...
function _defaultPluginPaths(type) {
    ...
    if (process.platform === 'win32')  return '%COMMONPROGRAMFILES%\\VST3:%COMMONPROGRAMFILES(X86)%\\VST3';
    if (process.platform === 'darwin') return '~/Library/Audio/Plug-Ins/VST3:/Library/Audio/Plug-Ins/VST3';
    return '~/.vst3:/usr/lib/vst3:/usr/local/lib/vst3';
}
let lv2Paths=_defaultPluginPaths('lv2');
let vst3Paths=_defaultPluginPaths('vst3');
let projetName='';
...
async function openSpectWamWithBuffer(wavAB, sampleRate, canal, mode) {
	const payload = { type: "setSourceBuffer", buffer: wavAB, canal, mode, sampleRate };
	if (winSpectWamEtat === 0) {
		winSpectWam = new BrowserWindow({ width: 680, height: 530, alwaysOnTop: true,
			webPreferences: {
  ...
  if (typeof args === "string") {
	...
	switch(cmd[0]) {
		...
		case "spatialDefaut":
			...
			break;
		case "trajectoireDefaut":
			if(winTrajectoireEtat==1){
				winTrajectoire.webContents.send("fromMain", "defautTrajectoire");
			}
			break;
		...
		case 'traj3dCursor':
			if(winStudio3DEtat==1){
				winStudio3D.webContents.send("fromMain", "cursorTraj;"+cmd[1]+";"+cmd[2]+";"+cmd[3]+";"+cmd[4]+";"+cmd[5]);
			}
			break;
		case 'selectImg':
			selectImg();
...
// Retourne la liste de tous les plugins LV2 installés
ipcMain.handle('lv2-list', async () => {
    const { stdout } = await execFileAsync('lv2ls', [], { maxBuffer: 4 * 1024 * 1024 });
