...
class moogLadderNode extends AudioWorkletNode {

    constructor(context, baseURL, options) {
        super(context, 'moogLadder', options);
    ...
    ctrlChange(channel, ctrl, value) {
        if (this.fCtrlLabel[ctrl] !== []) {
            for (var i = 0; i < this.fCtrlLabel[ctrl].length; i++) {
                ...
                this.setParamValue(path, moogLadderNode.remap(value, 0, 127, this.fCtrlLabel[ctrl][i].min, this.fCtrlLabel[ctrl][i].max));
                if (this.output_handler) {
    ...
    pitchWheel(channel, wheel) {
        for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
            ...
            this.setParamValue(pw.path, moogLadderNode.remap(wheel, 0, 16383, pw.min, pw.max));
            if (this.output_handler) {
...
class moogLadder {
    ...
    async load() {
        try {
            ...
            let real_url = (this.baseURL === "") ? "moogLadder.wasm" : (this.baseURL + "/moogLadder.wasm");
            ...
            if (this.fWorkletProcessors.indexOf(name) === -1) {
                try {
                    let re = /JSON_STR/g;
                    let moogLadderProcessorString1 = moogLadderProcessorString.replace(re, json);
                    let real_url = window.URL.createObjectURL(new Blob([moogLadderProcessorString1], { type: 'text/javascript' }));
            ...
            this.node = new moogLadderNode(this.context, this.baseURL,
                {
            ...
            this.node.onprocessorerror = () => { console.log('An error from moogLadder-processor was detected.'); }
            return (this.node);
        } catch (e) {
            console.error(e);
            console.error("Faust " + this.name + " cannot be loaded or compiled");
            ...
            try {
                ...
                if (!this.linkExists(real_url)) {
                    ...
                    link.onload = (e) => {
                        ...
                        var element = createmoogLadderGUI(this.node);
                        resolve(element);
                    }
                } else {
...
let moogLadderProcessorString = `
    ...
    class moogLadderProcessor extends AudioWorkletProcessor {
        ...
        static get parameterDescriptors() 
        {
            ...
            var params = [];
            moogLadderProcessor.parse_ui(JSON.parse(\`JSON_STR\`).ui, params, moogLadderProcessor.parse_item1);
            return params;
        }
...
const dspName13 = "moogLadder";

// WAP factory or npm package module
if (typeof module === "undefined") {
    window.moogLadder = moogLadder;
    window.FaustmoogLadder = moogLadder;
    window[dspName13] = moogLadder;
} else {
    module.exports = { moogLadder };
}
