...
class oberheimBPFNode extends AudioWorkletNode {

    constructor(context, baseURL, options) {
        super(context, 'oberheimBPF', options);
    ...
    ctrlChange(channel, ctrl, value) {
        if (this.fCtrlLabel[ctrl] !== []) {
            for (var i = 0; i < this.fCtrlLabel[ctrl].length; i++) {
                ...
                this.setParamValue(path, oberheimBPFNode.remap(value, 0, 127, this.fCtrlLabel[ctrl][i].min, this.fCtrlLabel[ctrl][i].max));
                if (this.output_handler) {
    ...
    pitchWheel(channel, wheel) {
        for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
            ...
            this.setParamValue(pw.path, oberheimBPFNode.remap(wheel, 0, 16383, pw.min, pw.max));
            if (this.output_handler) {
...
class oberheimBPF {
    ...
    async load() {
        try {
            ...
            let real_url = (this.baseURL === "") ? "oberheimBPF.wasm" : (this.baseURL + "/oberheimBPF.wasm");
            ...
            if (this.fWorkletProcessors.indexOf(name) === -1) {
                try {
                    let re = /JSON_STR/g;
                    let oberheimBPFProcessorString1 = oberheimBPFProcessorString.replace(re, json);
                    let real_url = window.URL.createObjectURL(new Blob([oberheimBPFProcessorString1], { type: 'text/javascript' }));
            ...
            this.node = new oberheimBPFNode(this.context, this.baseURL,
                {
            ...
            this.node.onprocessorerror = () => { console.log('An error from oberheimBPF-processor was detected.'); }
            return (this.node);
        } catch (e) {
            console.error(e);
            console.error("Faust " + this.name + " cannot be loaded or compiled");
            ...
            try {
                ...
                if (!this.linkExists(real_url)) {
                    ...
                    link.onload = (e) => {
                        ...
                        var element = createoberheimBPFGUI(this.node);
                        resolve(element);
                    }
                } else {
...
let oberheimBPFProcessorString = `
    ...
    class oberheimBPFProcessor extends AudioWorkletProcessor {
        ...
        static get parameterDescriptors() 
        {
            ...
            var params = [];
            oberheimBPFProcessor.parse_ui(JSON.parse(\`JSON_STR\`).ui, params, oberheimBPFProcessor.parse_item1);
            return params;
        }
...
const dspName2 = "oberheimBPF";

// WAP factory or npm package module
if (typeof module === "undefined") {
    window.oberheimBPF = oberheimBPF;
    window.FaustoberheimBPF = oberheimBPF;
    window[dspName2] = oberheimBPF;
} else {
    module.exports = { oberheimBPF };
}
