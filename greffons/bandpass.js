...
class bandpassNode extends AudioWorkletNode {
    ...
    ctrlChange(channel, ctrl, value) {
        if (this.fCtrlLabel[ctrl] !== []) {
            for (var i = 0; i < this.fCtrlLabel[ctrl].length; i++) {
                ...
                this.setParamValue(path, bandpassNode.remap(value, 0, 127, this.fCtrlLabel[ctrl][i].min, this.fCtrlLabel[ctrl][i].max));
                if (this.output_handler) {
    ...
    pitchWheel(channel, wheel) {
        for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
            ...
            this.setParamValue(pw.path, bandpassNode.remap(wheel, 0, 16383, pw.min, pw.max));
            if (this.output_handler) {
...
class bandpass {
    ...
    async load() {
        try {
            ...
            let real_url = (this.baseURL === "") ? "bandpass.wasm" : (this.baseURL + "/bandpass.wasm");
            ...
            if (this.fWorkletProcessors.indexOf(name) === -1) {
                try {
                    let re = /JSON_STR/g;
                    let bandpassProcessorString1 = bandpassProcessorString.replace(re, json);
                    let real_url = window.URL.createObjectURL(new Blob([bandpassProcessorString1], { type: 'text/javascript' }));
            ...
            this.node = new bandpassNode(this.context, this.baseURL,
                {
            ...
            this.node.onprocessorerror = () => { console.log('An error from bandpass-processor was detected.'); }
            return (this.node);
        } catch (e) {
            console.error(e);
            console.error("Faust " + this.name + " cannot be loaded or compiled");
            ...
            try {
                ...
                if (!this.linkExists(real_url)) {
                    ...
                    link.onload = (e) => {
                        ...
                        var element = createbandpassGUI(this.node);
                        resolve(element);
                    }
                } else {
...
let bandpassProcessorString = `
    ...
    class bandpassProcessor extends AudioWorkletProcessor {
        ...
        static get parameterDescriptors() 
        {
            ...
            var params = [];
            bandpassProcessor.parse_ui(JSON.parse(\`JSON_STR\`).ui, params, bandpassProcessor.parse_item1);
            return params;
        }
...
const dspName1 = "bandpass";

// WAP factory or npm package module
if (typeof module === "undefined") {
    window.bandpass = bandpass;
    window.Faustbandpass = bandpass;
    window[dspName1] = bandpass;
} else {
