...
class ringmodulatorNode extends AudioWorkletNode {

    constructor(context, baseURL, options) {
        super(context, 'ringmodulator', options);
    ...
    ctrlChange(channel, ctrl, value) {
        if (this.fCtrlLabel[ctrl] !== []) {
            for (var i = 0; i < this.fCtrlLabel[ctrl].length; i++) {
                ...
                this.setParamValue(path, ringmodulatorNode.remap(value, 0, 127, this.fCtrlLabel[ctrl][i].min, this.fCtrlLabel[ctrl][i].max));
                if (this.output_handler) {
    ...
    pitchWheel(channel, wheel) {
        for (var i = 0; i < this.fPitchwheelLabel.length; i++) {
            ...
            this.setParamValue(pw.path, ringmodulatorNode.remap(wheel, 0, 16383, pw.min, pw.max));
            if (this.output_handler) {
...
class ringmodulator {
    ...
    async load() {
        try {
            ...
            let real_url = (this.baseURL === "") ? "ringmodulator.wasm" : (this.baseURL + "/ringmodulator.wasm");
            ...
            if (this.fWorkletProcessors.indexOf(name) === -1) {
                try {
                    let re = /JSON_STR/g;
                    let ringmodulatorProcessorString1 = ringmodulatorProcessorString.replace(re, json);
                    let real_url = window.URL.createObjectURL(new Blob([ringmodulatorProcessorString1], { type: 'text/javascript' }));
            ...
            this.node = new ringmodulatorNode(this.context, this.baseURL,
                {
            ...
            this.node.onprocessorerror = () => { console.log('An error from ringmodulator-processor was detected.'); }
            return (this.node);
        } catch (e) {
            console.error(e);
            console.error("Faust " + this.name + " cannot be loaded or compiled");
            ...
            try {
                ...
                if (!this.linkExists(real_url)) {
                    ...
                    link.onload = (e) => {
                        ...
                        var element = createringmodulatorGUI(this.node);
                        resolve(element);
                    }
                } else {
...
let ringmodulatorProcessorString = `
    ...
    class ringmodulatorProcessor extends AudioWorkletProcessor {
        ...
        static get parameterDescriptors() 
        {
            ...
            var params = [];
            ringmodulatorProcessor.parse_ui(JSON.parse(\`JSON_STR\`).ui, params, ringmodulatorProcessor.parse_item1);
            return params;
        }
...
const dspName18 = "ringmodulator";

// WAP factory or npm package module
if (typeof module === "undefined") {
    window.ringmodulator = ringmodulator;
    window.Faustringmodulator = ringmodulator;
    window[dspName18] = ringmodulator;
} else {
