...
function defSymbGrp(id,nbobjets) {
	...
	if(id.etat==1){
		...
		if(parseInt(id.type)==19||parseInt(id.type)==20||parseInt(id.type)==71||parseInt(id.type)==72){
			txt=txt+"<x3 value='"+(id.x3||0)+"'></x3>\n\
			<y3 value='"+(id.y3||0)+"'></y3>\n";
		}
		if(parseInt(id.type)==25){
			txt=txt+"<curveh value='"+(id.curveH!==undefined?id.curveH:10)+"'></curveh>\n";
		}
...
function defProjetConf(txt) {
	...
	<externemcmidiplayerpath value='"+mcMidiPlayerPath+"'></externemcmidiplayerpath>\n\
...
function saveProjetA(t,offset,tabgrp,svgB64){
	...
	for(let i=0;i<ntable.length;i++){
		...
		if(ntable[i].etat==1 && (ntable[i].class==4)){
...
function symbXmlToScore(id,i) {
	...
	tableObjet[id]={
		...
		if(tableObjet[id].type==25){
			var _ch=org.getElementsByTagName("curveh")[0];
			tableObjet[id].curveH=_ch?parseFloat(_ch.getAttribute("value")):10;
		}
			...
			(async()=>{
				...
				for(let k=0;k<nbObjets;k++){
					if(tableObjet[k] && tableObjet[k].class===1 && tableObjet[k].file && tableObjet[k].file!==""){
						try{ await generateFxWavForLoad(k); }
						catch(e){ console.warn('[load] generateFxWavForLoad objet'+k, e.message||e); }
					}
				}
			})();
}
...
function defExterneConfig() {
	if(document.getElementById("fichierSave").getElementsByTagName("kandiskyscore")[0].getElementsByTagName("externe")[0]){
		...
		mcMidiPlayerPath = _tagVal(obj, "externemcmidiplayerpath") || '';
...
if (theme[49]) paletteNuage=theme[49];
if (theme[50]) paletteImage=theme[50];
if (theme[51]) paletteSymb=theme[51];
if (theme[52]) paletteFleche=theme[52];
if (theme[53]) paletteMarque1=theme[53];
if (theme[54]) paletteMarque2=theme[54];
if (theme[55]) paletteLecteur=theme[55];
