...
for iline=4, #lines,1 do
  ...
  reaper.ShowConsoleMsg("nbtrack " .. numtracks .."\n")
  local ltrack=tonumber(items[4])
  if ltrack+baseTrack>numtracks then
       reaper.InsertTrackAtIndex(numtracks+1+baseTrack, true)
       reaper.TrackFX_AddByName(reaper.GetTrack(0, tonumber(items[4])-1+baseTrack),lines[2],false,1)
       reaper.ShowConsoleMsg("nbtrack2 " .. numtracks )
  end
  local track = reaper.GetTrack(0, tonumber(items[4])-1+baseTrack) -- track 1  
  local mediaFilePath = directory .. "exports/" .. items[3] .. ".wav"
  local pos=tonumber(items[5])/48000
  ...
  reaper.ShowConsoleMsg("length " .. retval .. "\n")
  ...
  local spaced={}
  reaper.ShowConsoleMsg("gmatch " .. items[10] .. "\n")
  ...
  for ifx=1, #spacet,1 do
    -- reaper.SetEnvelopePoint(envx, 0,pos+tonumber(spacet[ifx]), tonumber(spacex[ifx]), 0, 0, false, false)
